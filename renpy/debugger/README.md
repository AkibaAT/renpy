...
17. [Advanced Debugger Views](#advanced-debugger-views)
18. [Live Editing](#live-editing)
...
- VSCode with the Ren'Py Language extension on the `feature/debugger` branch or a release that includes the `renpy` debug adapter and advanced debugger views
...
## Advanced Debugger Views

The Ren'Py Language extension adds additional debug sidebar views when an active `renpy` debug session is running. These views use custom DAP requests implemented by the engine debugger.

### Rollback History

- Lists rollback checkpoints and the current execution position.
- Can include non-checkpoint rollback entries when "Show All" is enabled.
- Can roll back to hard checkpoints and search for changes to a selected variable.
- Shows a "No active debug session" or "No rollback history available" state when the game is not ready.

### Test Recorder

- Starts and stops interaction recordings.
- Captures screenshots and variable assertions while recording.
- Lists saved recordings and can play, delete, or export them.
- Playback verification reports assertion and screenshot pass/fail counts.

### Save Inspector

- Lists save slots, persistent variables, and preferences.
- Opens save details as JSON.
- Compares two save slots.
- Edits or resets persistent variables and can force persistent data to save.

### Layered Images

- Lists all layered images known to Ren'Py.
- Lists currently shown layered images.
- Opens image details as JSON.
- Toggles attributes on shown layered images and previews attribute combinations.

### Compatibility Contract

The extension and engine must agree on these custom DAP request names:

`getRollbackHistory`, `gotoCheckpoint`, `findVariableChanges`, `getRecordingStatus`, `getPlaybackStatus`, `listRecordings`, `startRecording`, `stopRecording`, `captureScreenshot`, `addAssertion`, `playRecording`, `stopPlayback`, `deleteRecording`, `exportRecording`, `listSaves`, `getPersistentData`, `getSaveDetails`, `compareSaves`, `setPersistent`, `deletePersistent`, `getLayeredImages`, `getShownLayeredImages`, `getLayeredImageDetails`, `setLayeredImageAttribute`, and `previewLayeredImage`.

---

## Live Editing

Live editing updates script and asset changes without requiring a full game restart when the debugger is running.

### Supported Changes

- Dialogue text and speaker changes for Say and TranslateSay statements.
- Say statements with character attributes, such as `e happy "Text"` and `e @ happy "Text"`.
- Show, scene, hide, play, screen, transform, style, define, default, menu, and Python statement updates where the active AST node can be updated safely.
- Image, audio, font, and shader asset refreshes.

### Limitations

- Complex structural edits may still require a normal Ren'Py script reload.
- Rollback availability controls how much of the current interaction can be replayed after an edit.
- Persistent data changes are not rolled back by live edit.
