...
def enable_live_edit() -> bool:
    """
    Enable live editing of dialogue text.

    When enabled, edits to the currently displayed Say statement
    are applied immediately without a full script reload.

    Returns:
        True if live editing was enabled, False if debugger not running.
    """
    debugger = get_debugger()
    if debugger is None:
        return False
    debugger.live_edit_manager.enable()
    return True


def disable_live_edit() -> bool:
    """
    Disable live editing.

    Returns:
        True if live editing was disabled, False if debugger not running.
    """
    debugger = get_debugger()
    if debugger is None:
        return False
    debugger.live_edit_manager.disable()
    return True


def is_live_edit_enabled() -> bool:
    """
    Check if live editing is enabled.

    Returns:
        True if live editing is enabled, False otherwise.
    """
    debugger = get_debugger()
    if debugger is None:
        return False
    return debugger.live_edit_manager.is_enabled
